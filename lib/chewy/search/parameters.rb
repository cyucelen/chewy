...
module Chewy
  module Search
    ...
    protected
      ...
      def render_body
        exceptions = %i[filter query none function_score score_mode boost_mode] + QUERY_STRING_STORAGES
      ...
      def render_query
        ...
        boost_mode = @storages[:boost_mode].render

        if function_score
          [query, filter, score_mode, boost_mode].each do |storage|
